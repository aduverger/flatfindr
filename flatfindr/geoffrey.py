"""
Spinn-off Alfred, which can search for pretty much anything you want.
...
def start(update: Update, context: CallbackContext) -> int:
    ...
    update.message.reply_text(
        ...
        reply_markup=ReplyKeyboardMarkup(
            ...
            input_field_placeholder="Categories",
        ),
    )
...
def stop(update: Update, context: CallbackContext) -> None:
    ...
    text = (
        "Search successfully cancelled!"
        if job_removed
        else "You have no active search."
    )
...
def main() -> None:
    ...
    persistence = PicklePersistence(
        filename=os.path.join(
            ...
            os.path.join("saves", "geoffrey.pkl"),
        )
    )
    updater = Updater(LOGINS["geoffrey"], use_context=True, persistence=persistence)
